...
public class RecoverableStrongTreeMap<K extends OffHeapObject, V extends OffHeapObject>
        extends AbstractMap<K,V> implements PersistentMap<K,V>, OffHeapObject {

    private static final long CLASS_ID = OffHeap.Klass.register( RecoverableStrongTreeMap.class );
    ...
    public static class OffHeapNode<K extends OffHeapObject, V extends OffHeapObject>
        ...
        private static final long CLASS_ID = OffHeap.Klass.register( RecoverableStrongTreeMap.OffHeapNode.class );
...
*/
    ...
    //Constructor
    public RecoverableStrongTreeMap() {
        this( DEFAULT_SIZE );
    }

    public RecoverableStrongTreeMap(Map<? extends K, ? extends V> m) {
        this( m.size() );
        putAll( m );
    }

    public RecoverableStrongTreeMap(int initialSize) {
    ...
    //Reconstructor
    public RecoverableStrongTreeMap(long offset) {
...
*/
    ...
    public static RecoverableStrongTreeMap recover(String name, int initialSize) {
        ...
        RecoverableStrongTreeMap ret = null;
        if( oho != null ) {
          if( oho instanceof RecoverableStrongTreeMap ) {
            ret = (RecoverableStrongTreeMap) oho;
          } else {
            throw new IllegalStateException("Root name already exists");
          }
        } else {
          ret = new RecoverableStrongTreeMap( initialSize );
