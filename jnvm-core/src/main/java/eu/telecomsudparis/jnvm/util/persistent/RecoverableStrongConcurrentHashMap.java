...
public class RecoverableStrongConcurrentHashMap<K extends OffHeapObject, V extends OffHeapObject>
        extends AbstractMap<K,V> implements RecoverableMap<K,V> {

    private static final long CLASS_ID = OffHeap.Klass.register( RecoverableStrongConcurrentHashMap.class );

    private static final int DEFAULT_SIZE = 16;

    private transient Queue<Long> reclaimed;
    private transient ConcurrentHashMap<K,OffHeapNode<K,V>> index;
    private OffHeapArray<OffHeapNode<K,V>> table;
    private transient ThreadLocal<V> returnValue = ThreadLocal.withInitial( () -> { return null; } );
    ...
    public static class OffHeapNode<K extends OffHeapObject, V extends OffHeapObject>
        ...
        private static final long CLASS_ID = OffHeap.Klass.register( RecoverableStrongConcurrentHashMap.OffHeapNode.class );
    ...
    //Constructor
    public RecoverableStrongConcurrentHashMap() {
        this( DEFAULT_SIZE );
    }

    public RecoverableStrongConcurrentHashMap(Map<? extends K, ? extends V> m) {
        this( m.size() );
        putAll( m );
    }

    public RecoverableStrongConcurrentHashMap(int initialSize) {
    ...
    //Reconstructor
    public RecoverableStrongConcurrentHashMap(long offset) {
...
*/
    ...
    public RecoverableStrongConcurrentHashMap(MemoryBlockHandle block) {
        this( block.getOffset() );
    }
    public RecoverableStrongConcurrentHashMap(Void v, long offset) {
        this( offset );
    }
    public static RecoverableStrongConcurrentHashMap recover(String name, int initialSize) {
        OffHeapObject oho = OffHeap.rootInstances.get( name );
        RecoverableStrongConcurrentHashMap ret = null;
        if( oho != null ) {
          if( oho instanceof RecoverableStrongConcurrentHashMap ) {
            ret = (RecoverableStrongConcurrentHashMap) oho;
          } else {
            throw new IllegalStateException("Root name already exists");
          }
        } else {
          ret = new RecoverableStrongConcurrentHashMap( initialSize );
    ...
    public V put(K key, V value) {
        ...
        index.compute( (K) key, ( k, entry ) -> {
            if( entry == null ) {
                ...
                V prev = entry.setValue( value );
                returnValue.set( prev );
            }
            return entry;
        } );
    ...
    public V replaceValue(Object key, V value) {
        checkIndexNonNull();
        returnValue.remove();
        index.computeIfPresent2( key, ( k, entry ) -> {
            V prev = entry.setValue( value );
            returnValue.set( prev );
            return entry;
        } );
    ...
    public V get(Object key) {
        ...
        returnValue.remove();
        index.computeIfPresent2( key, ( k, entry ) -> {
            returnValue.set( entry.getValue() );
            return entry;
        } );
        return returnValue.get();
    }
    ...
    public V remove(Object key) {
        ...
        index.computeIfPresent2( key, ( k, entry ) -> {
            returnValue.set( entry.getValue() );
            long tableIndex = entry.tableIndex;
            entry.invalidate();
            table.clear( tableIndex );
            reclaimed.add( tableIndex );
            //Returning null removes the key and value from index
            return null;
        } );
        return returnValue.get();
    }
