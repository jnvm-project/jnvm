...
public class RecoverableStrongSkipListMap<K extends OffHeapObject, V extends OffHeapObject>
        extends AbstractMap<K,V> implements RecoverableMap<K,V> {

    private static final long CLASS_ID = OffHeap.Klass.register( RecoverableStrongSkipListMap.class );
    ...
    public static class OffHeapNode<K extends OffHeapObject, V extends OffHeapObject>
        ...
        private static final long CLASS_ID = OffHeap.Klass.register( RecoverableStrongSkipListMap.OffHeapNode.class );
...
*/
    ...
    //Constructor
    public RecoverableStrongSkipListMap() {
        this( DEFAULT_SIZE );
    }

    public RecoverableStrongSkipListMap(Map<? extends K, ? extends V> m) {
        this( m.size() );
        putAll( m );
    }

    public RecoverableStrongSkipListMap(int initialSize) {
    ...
    //Reconstructor
    public RecoverableStrongSkipListMap(long offset) {
...
*/
    ...
    public static RecoverableStrongSkipListMap recover(String name, int initialSize) {
        ...
        RecoverableStrongSkipListMap ret = null;
        if( oho != null ) {
          if( oho instanceof RecoverableStrongSkipListMap ) {
            ret = (RecoverableStrongSkipListMap) oho;
          } else {
            throw new IllegalStateException("Root name already exists");
          }
        } else {
          ret = new RecoverableStrongSkipListMap( initialSize );
